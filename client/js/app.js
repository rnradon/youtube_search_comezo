(function(){
    ...
    app.config(function($routeProvider, $locationProvider, $httpProvider) {
        ...
        $routeProvider
            .when("/", {
                templateUrl : "views/search_youtube.html",
                // showHeader : false,
            })
            .when("/video_id/:videoId", {
                templateUrl : "views/video_tags.html",
                
                controller: 'video_tags'
            })
            .when("/all_videos", {
                templateUrl : "views/all_videos.html",
     ...
     app.controller('video_tags', ['$routeParams', '$scope', '$http', function($routeParams, $scope, $http, $rootScope ) {
        
        var video_id = $routeParams.videoId
        alert(video_id)
        $scope.id;
        $http.get('/api/Videos/findOne?filter[where][video_id]=' + video_id )
        .then(function(response) {
        $scope.videos_json_data = response.data;
        $scope.id = response.data.id;
        listAllTags($scope.id);
        });
        console.log("------")
        console.log($scope.id);
        function listAllTags(id){
        $http.get('/api/Videos/' + id +'/tags')
        .then(function(response) {
        $scope.tags_json_data = response.data;
     ...
     app.controller('all_videos', ['$routeParams', '$scope', '$http', function($routeParams, $scope, $http, $rootScope ) {
        
        
        $http.get('/api/Videos' )
        .then(function(response) {
        $scope.all_videos_json_data = response.data;
